import React, { useState, useRef, useEffect } from "react";// some think imported 
import {
  View,
  Text,
  StyleSheet,
...
export default function ChatbotScreen() {
    ...
    const loadSettings = async () => {
      try {
        const [storedIp, storedPort] = await Promise.all([
          AsyncStorage.getItem("scrapit.serverIp"),
          AsyncStorage.getItem("scrapit.serverPort"),
        ]);
  ...
  useEffect(() => {
    ...
    void AsyncStorage.setItem("scrapit.serverIp", serverIp);
  }, [serverIp]);

  useEffect(() => {
    if (!hasLoadedSettingsRef.current) return;
    void AsyncStorage.setItem("scrapit.serverPort", serverPort);
  }, [serverPort]);
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    try {
      ...
      errorMessageText +=
        "\n\nHow to fix this:\n" +
        "1. Ensure your FastAPI server is running (`uvicorn main:app --app-dir backend --reload`)\n" +
        `2. Check if your laptop IP is still ${serverIp}\n` +
        "3. Make sure your laptop and mobile are on the SAME Wi-Fi.\n" +
        `4. Run your server with: \`uvicorn main:app --app-dir backend --host 0.0.0.0 --port ${serverPort}\``;
