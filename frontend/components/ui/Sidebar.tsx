...
import ModernAlert, { ModernAlertProps } from "./ModernAlert";
...
export const Sidebar: React.FC<SidebarProps> = ({ isVisible, onClose }) => {
  ...
  const handlePageClick = (id: string) => {
    ...
    if (id.startsWith("acc-") || id === "notifications" || id === "comm-chat") {
      router.push({
        pathname: "/settings",
        params: { tab },
      });
    ...
    if (id === "favourites") alert("Favourites coming soon!");
    if (id === "cart") router.push("/cart");
    if (id === "logout") {
      setAlertConfig({
        title: "Log Out",
        message: "Are you sure you want to sign out? You will need to log back in to access your data.",
        type: "danger",
        showCancel: true,
