    ...
    for (const a of anchors) {
        try {
            ...
            for (let k = lines.length - 1; k >= 0; k--) {
                if (lines[k].length > 60 && !/apply/i.test(lines[k])) { snippet = lines[k]; break; }
            }

            const img = card.querySelector('img');
            let logo = img ? (img.getAttribute('data-src') || img.src || '') : '';
            if (/pixel|tracker|blank|placeholder/i.test(logo)) logo = '';
...
_DETAIL_JS_TEMPLATE = r"""(companyName) => {
    ...
    const sels = [
        ...
        '[class*="job-detail"]', '[class*="detail-content"]', '[class*="description"]', 'article'
    ];
    let description = '';
    for (const s of sels) {
        const t = text(s);
        if (t && t.length > 120) { description = t; break; }
    }
    if (!description) {
        ...
        document.querySelectorAll('div, section, p').forEach(el => {
            ...
            if (t.length > best.length && t.length < 6000 && t.split(' ').length > 25) best = t;
        });
        description = best;
    }

    // Logo: exclude sidebar / recommended / related card containers
    // Strategy 1: match img.alt against company name (most reliable)
    let logo = '';
    const cname = (companyName || '').toLowerCase().trim();
    if (cname) {
        const allImgs = Array.from(document.querySelectorAll('img[alt]'));
        const matched = allImgs.find(img => {
            const alt = (img.getAttribute('alt') || '').toLowerCase();
            return alt && cname && (alt.includes(cname) || cname.includes(alt));
        });
        if (matched) logo = matched.getAttribute('src') || matched.getAttribute('data-src') || '';
    }

    // Strategy 2: look for logo-classed images but NOT inside sidebar/recommended areas
    if (!logo) {
        const sidebarSelectors = [
            '[class*="sidebar"]', '[class*="recommend"]', '[class*="related"]',
            '[class*="similar"]', '[class*="suggested"]', '[id*="sidebar"]'
        ];
        const excluded = new Set();
        sidebarSelectors.forEach(sel => {
            document.querySelectorAll(sel).forEach(el => {
                el.querySelectorAll('img').forEach(img => excluded.add(img));
            });
        });

        const logoImgs = Array.from(document.querySelectorAll(
            'img[class*="logo"], .company-logo img, [class*="company"] img, img[src*="company"], img[src*="logo"]'
        ));
        const validLogo = logoImgs.find(img => !excluded.has(img));
        if (validLogo) logo = validLogo.getAttribute('src') || validLogo.getAttribute('data-src') || '';
    }
