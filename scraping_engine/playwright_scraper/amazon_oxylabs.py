...
def _looks_like_product(item: dict) -> bool:
    keys = set(item.keys())
    return bool(
        {"asin", "title", "name", "url", "price", "rating", "reviews_count"} & keys
    ) and not {"refinement_display_name", "value"} <= keys
...
def _coerce_search_items(content: Any) -> List[Dict[str, Any]]:
    ...
    def walk(value: Any):
        if isinstance(value, list):
            product_like = [x for x in value if isinstance(x, dict) and _looks_like_product(x)]
            if len(product_like) >= 2:
                candidates.extend(product_like)
                return
            for item in value:
                walk(item)
    ...
    for item in candidates:
        identity = item.get("asin") or item.get("url") or item.get("title") or item.get("name")
...
def scrape_amazon_products(query: str, max_products: int = 20, geo_location: str = "90210") -> Dict[str, Any]:
    payload = {
    ...
    try:
        ...
        logger.info("[Amazon/Oxylabs] Raw response keys: %s", oxylabs_result.keys())
        # Extract results list and log details for debugging
        results = oxylabs_result.get("results") or []
        logger.info("[Amazon/Oxylabs] Results len: %s", len(results))
        if results:
            logger.info("[Amazon/Oxylabs] Result[0] keys: %s", results[0].keys())
            logger.info("[Amazon/Oxylabs] Content sample: %s", str(results[0].get("content"))[:2000])
    ...
    for raw in items[:max_products]:
        ...
        products.append(product_payload(
            ...
            image_url=raw.get("url_image") or raw.get("image") or raw.get("image_url") or raw.get("thumbnail") or "",
...
def scrape_amazon_detail(url_or_asin: str, geo_location: str = "90210") -> Dict[str, Any]:
    query = url_or_asin
    payload = {
